      ...
      <aside className="w-52 shrink-0 border-r border-gray-200 bg-white overflow-y-auto">
        ...
        {loadingMonths ? (
          ...
          <div className="px-2 py-3 space-y-4">
            {years.map((year) => (
              <div key={year}>
                <p className="px-2 mb-1 text-xs font-semibold text-gray-400 uppercase tracking-wide">{year}</p>
            ...
            <div>
              <div className="flex items-baseline justify-between mb-3">
                ...
                <div className="flex items-baseline gap-3">
                  <span className="text-xs text-gray-400">{report.summary.transaction_count} transactions</span>
                  <Link
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="rounded-lg border border-gray-200 bg-white p-4">
                  ...
                  <p className="text-lg font-semibold text-green-700">{formatCurrency(report.summary.income)}</p>
                </div>
                <div className="rounded-lg border border-gray-200 bg-white p-4">
                  <p className="text-xs text-gray-500 mb-1">Expenses</p>
                  <p className="text-lg font-semibold text-red-600">{formatCurrency(report.summary.expenses)}</p>
                </div>
                <div className="rounded-lg border border-gray-200 bg-white p-4">
                  ...
                  <table className="w-full text-sm">
                    <tbody>
                      {report.category_breakdown.map((cat) => (
                        <>
                          <tr key={cat.category} className="bg-gray-100 border-t border-gray-200 first:border-t-0">
                          ...
                          {cat.subcategories.map((sub) => (
                            <tr key={`${cat.category}-${sub.subcategory}`} className="bg-white border-t border-gray-100">
